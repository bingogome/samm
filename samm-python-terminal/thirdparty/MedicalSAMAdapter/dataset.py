...
class REFUGE(Dataset):
    def __init__(
        ...
        self.subfolders = [
            f.path
            for f in os.scandir(os.path.join(data_path, mode + "-400"))
            if f.is_dir()
        ]
        self.mode = mode
        self.prompt = prompt
    ...
    def __getitem__(self, index):
        ...
        img = Image.open(img_path).convert("RGB")
        multi_rater_cup = [
            Image.open(path).convert("L") for path in multi_rater_cup_path
        ]
        multi_rater_disc = [
            Image.open(path).convert("L") for path in multi_rater_disc_path
        ]

        # resize raters images for generating initial point click
        newsize = (self.img_size, self.img_size)
        multi_rater_cup_np = [
            np.array(single_rater.resize(newsize)) for single_rater in multi_rater_cup
        ]
        multi_rater_disc_np = [
            np.array(single_rater.resize(newsize)) for single_rater in multi_rater_disc
        ]

        # first click is the target agreement among most raters
        if self.prompt == "click":
            point_label, pt_cup = random_click(
                np.array(np.mean(np.stack(multi_rater_cup_np), axis=0)) / 255,
                point_label,
            )
            point_label, pt_disc = random_click(
        ...
        if self.transform:
            ...
            multi_rater_cup = [
                torch.as_tensor(
                    (self.transform(single_rater) > 0.5).float(), dtype=torch.float32
                )
    ...
    def __getitem__(self, index):
        ...
        if self.prompt == "click":
            point_label, pt = random_click(
                np.array(np.mean(np.stack(multi_rater), axis=0)) / 255, point_label
            )
        ...
        multi_rater = [
            torch.from_numpy(single_rater).type(torch.float32)
            for single_rater in multi_rater
        ]
